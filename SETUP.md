...
#!/usr/bin/env bash
set -euo pipefail
name="${1:?usage: rollback-function.sh <function-name> <git-sha>}"
sha="${2:?usage: rollback-function.sh <function-name> <git-sha>}"
trap 'git checkout HEAD -- "supabase/functions/$name" 2>/dev/null || true' EXIT
